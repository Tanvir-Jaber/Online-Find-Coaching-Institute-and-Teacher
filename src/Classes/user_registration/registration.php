...
class registration extends DB
{
   public $name,$email,$phone,$position,$password,$address,$bloodGroup,$cname,$token,$date;
    ...
    public function insertRegisterData(){
            $dataArray=array($this->name,$this->email,$this->phone,$this->bloodGroup,$this->position,$this->cname,$this->password,$this->address,$this->token);
            $sql="insert into users(name,email,pnumber,nid,position,cname,pass,address,emailtoken)VALUES (?, ?, ?, ?, ?, ?, ?,?,?)";
